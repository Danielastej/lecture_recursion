import os
import json

cwd_path = os.getcwd()
file_path = "files"


def read_data(file_name, key="ordered_numbers"):
    """
    Reads json file and returns sequential data.
    :param file_name: (str), name of json file
    :param key: (str), field of a dict to return
    :return: (list, str), sequential data
    """
    if key not in {"unordered_numbers", "ordered_numbers", "dna_sequence"}:
        return None

    with open(os.path.join(cwd_path, file_path, file_name), mode="r") as json_file:
        seqs = json.load(json_file)

    return seqs[key]


def binary_search(seq, number):
    """
    Function performs binary search on !!ordered!! sequence and stores position of match if found.
    :param seq: (list): list of numbers
    :param number: (int): number to match within sequence
    :return: (int, None): index of match if found, None otherwise
    """
    left, right = (0, len(seq) - 1)

    while left <= right:
        middle = (right + left) // 2

        if number < seq[middle]:
            right = middle - 1
        elif number > seq[middle]:
            left = middle + 1
        else:
            return middle
    return None

def recursive_binary_search(seq, number, idx_left, idx_right):
    mid_idx = (idx_right + idx_left) // 2
    middle = seq[mid_idx]
    if middle == number:
        return mid_idx
    elif middle < number:
        return recursive_binary_search(seq, number, mid_idx + 1, idx_right)
    elif middle > number:
       return recursive_binary_search(seq, number, idx_left, mid_idx - 1)
    else:
        return None

def recursive_binary_search_2(seq, number, idx_left, idx_right):
    middle = (idx_right + idx_left) // 2
    if idx_left > idx_right:
        return None
    if number == seq[middle]:
        return middle
    elif idx_left == idx_right:
        return None
    elif number < seq[middle]:
        return recursive_binary_search_2(seq, number, idx_left, middle - 1)
    elif number > seq[middle]:
        return recursive_binary_search_2(seq, number, middle + 1, idx_right)



def main(file_name, number):
    sequence = read_data(file_name=file_name, key="ordered_numbers")

    # iterative binary search
    result = binary_search(sequence, number=number)
    print(f'Binary search result:{result}')


if __name__ == "__main__":
    my_file = "sequential.json"
    my_number = 90
    main(my_file, my_number)
